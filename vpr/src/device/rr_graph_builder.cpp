...
void RRGraphBuilder::add_node_to_all_locs(RRNodeId node) {
    ...
    for (int ix = node_storage_.node_xlow(node); ix <= node_storage_.node_xhigh_ptn(node, node_ptn); ix++) {
        for (int iy = node_storage_.node_ylow(node); iy <= node_storage_.node_yhigh_ptn(node, node_ptn); iy++) {
            switch (node_type) {
                case SOURCE:
                case SINK:
                case CHANY:
