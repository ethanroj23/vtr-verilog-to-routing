#ifndef PARTIAL_FOLDED_RR_GRAPH_H
#define PARTIAL_FOLDED_RR_GRAPH_H
...
class PartialFoldedRRGraph : public RRGraphViewInterface{
    /* -- Constructors -- */
  public:
    /* Explicitly define the only way to create an object */
    explicit PartialFoldedRRGraph(const t_rr_graph_storage& node_storage);
     ...
     * by compiler
     */
    PartialFoldedRRGraph(const PartialFoldedRRGraph&) = delete;
    void operator=(const PartialFoldedRRGraph&) = delete;

    /* -- Accessors -- */
  public:
  // every function takes in an RRNodeId that has not been remapped yet
  /* The general method for accessing data from the PartialFoldedRRGraph is as follows:
  ...
  // Estimate of memory taken by PartialFoldedRRGraph
  inline int memory_used() const {
        ...
        VTR_LOG("RRGraph[%s] Memory Breakdown (MiB): \ntile_patterns - %f\nnode_patterns - %f\nnode_pattern_data - %f\nremapped_ids - %f\nnode_to_x_y - %f\nnode_type - %f\nsize - %f\n",
             rr_graph_name(),
             tile_patterns_memory / MiB,
             node_patterns_memory / MiB,
             node_pattern_data_memory / MiB,
             remapped_ids_memory / MiB,
             node_to_x_y_memory / MiB,
             node_type_memory / MiB,
             size_memory / MiB );
  ...
  /* Pattern of data about a node. Many nodes will share the data within this struct and thus will have the same FoldedNodePattern */
    ...
    */
    ...
    bool built = false; // flag for determining if the PartialFoldedRRGraph has been built yet
