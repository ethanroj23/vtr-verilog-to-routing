    ...
    for (auto net_id : cluster_ctx.clb_nlist.nets()) {
        ...
        while (tptr != nullptr) {
            ...
            if (rr_type == SINK) {
                ...
                for (i = rr_graph.node_xlow((RRNodeId)inode); i <= rr_graph.node_xhigh_ptn((RRNodeId)inode, inode_ptn); i++)
                    chanx_occ[i][j]++;
            }

            else if (rr_type == CHANY) {
                i = rr_graph.node_xlow((RRNodeId)inode);
                for (j = rr_graph.node_ylow((RRNodeId)inode); j <= rr_graph.node_yhigh_ptn((RRNodeId)inode, inode_ptn); j++)
                    chany_occ[i][j]++;
            }
