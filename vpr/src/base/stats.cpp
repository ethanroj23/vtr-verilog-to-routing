    ...
    for (auto net_id : cluster_ctx.clb_nlist.nets()) {
        ...
        while (tptr != nullptr) {
            ...
            if (rr_type == SINK) {
                ...
                j = device_ctx.rr_graph.node_ylow(RRNodeId(inode)) /*ESR API*/;
                ...
                for (j = device_ctx.rr_graph.node_ylow(RRNodeId(inode)) /*ESR API*/; j <= device_ctx.rr_graph.node_yhigh(RRNodeId(inode)) /*ESR API*/; j++)
                    chany_occ[i][j]++;
            }
    ...
    while (tptr != nullptr) {
        ...
        if (curr_type == SINK) { /* Starting a new segment */
            ...
            length += 1 + device_ctx.rr_graph.node_xhigh(RRNodeId(inode)) /*ESR API*/ - device_ctx.rr_graph.node_xlow(RRNodeId(inode)) /*ESR API*/
                      + device_ctx.rr_graph.node_yhigh(RRNodeId(inode)) /*ESR API*/ - device_ctx.rr_graph.node_ylow(RRNodeId(inode)) /*ESR API*/;
