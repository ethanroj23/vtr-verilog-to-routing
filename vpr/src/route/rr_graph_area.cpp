        ...
        switch (from_rr_type) {
            ...
            case CHANY:
                ...
                for (iedge = 0; iedge < num_edges; iedge++) {
                    size_t to_node = device_ctx.rr_graph.node_edge_sink_node(RREdgeId(iedge)) /*ESR API*/;
                    ...
                    if ((device_ctx.rr_graph.node_type(RRNodeId(to_node)) /*ESR API*/ == SOURCE)
                        && (device_ctx.rr_graph.node_xlow(RRNodeId(to_node)) /*ESR API*/ == 0) && (device_ctx.rr_graph.node_ylow(RRNodeId(to_node)) /*ESR API*/ == 0)
                        && (device_ctx.rr_graph.node_xhigh(RRNodeId(to_node)) /*ESR API*/ == 0) && (device_ctx.rr_graph.node_yhigh(RRNodeId(to_node)) /*ESR API*/ == 0)) {
                        continue;
                    }
                    ...
                    switch (to_rr_type) {
                        case CHANX:
                        case CHANY:
                            iswitch = device_ctx.rr_graph.node_edge_switch(RREdgeId(iedge)) /*ESR API*/;
                ...
                if (from_rr_type == CHANX) {
                    ...
                    for (j = device_ctx.rr_graph.node_ylow(RRNodeId(from_node)) /*ESR API*/ - 1;
                         j <= device_ctx.rr_graph.node_yhigh(RRNodeId(from_node)) /*ESR API*/; j++) {
                        ntrans_sharing += shared_buffer_trans[j];
                        shared_buffer_trans[j] = 0.;
                    }

                    for (j = device_ctx.rr_graph.node_ylow(RRNodeId(from_node)) /*ESR API*/; j <= device_ctx.rr_graph.node_yhigh(RRNodeId(from_node)) /*ESR API*/;
                         j++)
                    ...
                    switch (to_rr_type) {
                        ...
                        case CHANY:
                            if (!chan_node_switch_done[to_node]) {
                                int switch_index = device_ctx.rr_graph.node_edge_switch(RREdgeId(iedge)) /*ESR API*/;
                                auto switch_type = device_ctx.rr_switch_inf[switch_index].type();

                                int fan_in = device_ctx.rr_graph.node_fan_in(RRNodeId(to_node));
