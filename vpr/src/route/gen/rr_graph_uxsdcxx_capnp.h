...
template <class T, typename Context>
void load_rr_switch_capnp_type(const ucap::RrSwitch::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack);
template <class T, typename Context>
void load_rr_switches_capnp_type(const ucap::RrSwitches::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack);
template <class T, typename Context>
...
template <class T, typename Context>
inline void write_rr_switches_capnp_type(T &in, ucap::RrSwitches::Builder &root, Context &context);
template <class T, typename Context>
...
template<class T, typename Context>
inline void load_rr_switch_capnp_type(const ucap::RrSwitch::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	(void)root;
	(void)out;
	(void)context;
	(void)report_error;
...
template<class T, typename Context>
inline void load_rr_switches_capnp_type(const ucap::RrSwitches::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	(void)root;
	(void)out;
	(void)context;
	(void)report_error;
	...
	stack->push_back(std::make_pair("getRrSwitch", 0));
	{
		auto data = root.getRrSwitches();
		out.preallocate_rr_switches_rr_switch(context, data.size());
		for(const auto & el : data) {
			auto child_context = out.add_rr_switches_rr_switch(context, el.getId());
			load_rr_switch_capnp_type(el, out, child_context, report_error, stack);
...
inline void load_rr_nodes_capnp_type(const ucap::RrNodes::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	...
	stack->push_back(std::make_pair("getNode", 0));
	{
		...
		for(const auto & el : data) {
			auto child_context = out.add_rr_nodes_node(context, el.getCapacity(), el.getId(), el.getSIdx(), conv_enum_node_type(el.getType(), report_error));
...
inline void load_rr_edges_capnp_type(const ucap::RrEdges::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	...
	stack->push_back(std::make_pair("getEdge", 0));
	{
		...
		for(const auto & el : data) {
			auto child_context = out.add_rr_edges_edge(context, el.getSinkNode(), el.getSrcNode());
...
inline void load_rr_graph_capnp_type(const ucap::RrGraph::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	...
	stack->pop_back();
	stack->push_back(std::make_pair("getRrSwitches", 0));
	if (root.hasRrSwitches()) {
		auto child_el = root.getRrSwitches();
		auto child_context = out.init_rr_graph_rr_switches(context);
		load_rr_switches_capnp_type(child_el, out, child_context, report_error, stack);
...
inline void write_rr_switches_capnp_type(T &in, ucap::RrSwitches::Builder &root, Context &context) {
	...
	size_t num_rr_switches_rr_switches = in.num_rr_switches_rr_switch(context);
	auto rr_switches_rr_switches = root.initRrSwitches(num_rr_switches_rr_switches);
	for(size_t i = 0; i < num_rr_switches_rr_switches; i++) {
		auto rr_switches_rr_switch = rr_switches_rr_switches[i];
		auto child_context = in.get_rr_switches_rr_switch(i, context);
		rr_switches_rr_switch.setId(in.get_rr_switch_id(child_context));
	}
}
...
inline void write_rr_graph_capnp_type(T &in, ucap::RrGraph::Builder &root, Context &context) {
...

	{
		...
		auto rr_graph_rr_switches = root.initRrSwitches();
		write_rr_switches_capnp_type(in, rr_graph_rr_switches, child_context);
	}
