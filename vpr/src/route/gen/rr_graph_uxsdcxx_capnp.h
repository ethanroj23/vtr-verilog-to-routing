...
template <class T, typename Context>
void load_dnode_capnp_type(const ucap::Dnode::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack);
template <class T, typename Context>
void load_shared_dnodes_capnp_type(const ucap::SharedDnodes::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack);
template <class T, typename Context>
void load_s_switch_capnp_type(const ucap::SSwitch::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack);
template <class T, typename Context>
void load_shared_switches_capnp_type(const ucap::SharedSwitches::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack);
template <class T, typename Context>
...
template <class T, typename Context>
inline void write_shared_dnodes_capnp_type(T &in, ucap::SharedDnodes::Builder &root, Context &context);
template <class T, typename Context>
inline void write_shared_switches_capnp_type(T &in, ucap::SharedSwitches::Builder &root, Context &context);
template <class T, typename Context>
...
inline void load_rr_nodes_capnp_type(const ucap::RrNodes::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	...
	stack->push_back(std::make_pair("getNode", 0));
	{
		...
		for(const auto & el : data) {
			auto child_context = out.add_rr_nodes_node(context, el.getCapacity(), el.getDIdx(), el.getId(), el.getSIdx(), conv_enum_node_type(el.getType(), report_error));
...
template<class T, typename Context>
inline void load_dnode_capnp_type(const ucap::Dnode::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	(void)root;
	(void)out;
	(void)context;
	(void)report_error;
...
template<class T, typename Context>
inline void load_shared_dnodes_capnp_type(const ucap::SharedDnodes::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	(void)root;
	(void)out;
	(void)context;
	(void)report_error;
	...
	stack->push_back(std::make_pair("getDnode", 0));
	{
		auto data = root.getDnodes();
		out.preallocate_shared_dnodes_dnode(context, data.size());
		for(const auto & el : data) {
			auto child_context = out.add_shared_dnodes_dnode(context, el.getVal());
			load_dnode_capnp_type(el, out, child_context, report_error, stack);
...
template<class T, typename Context>
inline void load_s_switch_capnp_type(const ucap::SSwitch::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	(void)root;
	(void)out;
	(void)context;
	(void)report_error;
...
template<class T, typename Context>
inline void load_shared_switches_capnp_type(const ucap::SharedSwitches::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	(void)root;
	(void)out;
	(void)context;
	(void)report_error;
	...
	stack->push_back(std::make_pair("getSSwitch", 0));
	{
		auto data = root.getSSwitches();
		out.preallocate_shared_switches_s_switch(context, data.size());
		for(const auto & el : data) {
			auto child_context = out.add_shared_switches_s_switch(context, el.getId());
			load_s_switch_capnp_type(el, out, child_context, report_error, stack);
...
inline void load_rr_edges_capnp_type(const ucap::RrEdges::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	...
	stack->push_back(std::make_pair("getSharedDnodes", 0));
	if (root.hasSharedDnodes()) {
		auto child_el = root.getSharedDnodes();
		auto child_context = out.init_rr_edges_shared_dnodes(context);
		load_shared_dnodes_capnp_type(child_el, out, child_context, report_error, stack);
		out.finish_rr_edges_shared_dnodes(child_context);
	}
	stack->pop_back();
	stack->push_back(std::make_pair("getSharedSwitches", 0));
	if (root.hasSharedSwitches()) {
		auto child_el = root.getSharedSwitches();
		auto child_context = out.init_rr_edges_shared_switches(context);
		load_shared_switches_capnp_type(child_el, out, child_context, report_error, stack);
...
inline void write_rr_nodes_capnp_type(T &in, ucap::RrNodes::Builder &root, Context &context) {
	...
	for(size_t i = 0; i < num_rr_nodes_nodes; i++) {
		...
		rr_nodes_node.setDIdx(in.get_node_d_idx(child_context));
		...
		rr_nodes_node.setSIdx(in.get_node_s_idx(child_context));
...
inline void write_shared_dnodes_capnp_type(T &in, ucap::SharedDnodes::Builder &root, Context &context) {
	...
	size_t num_shared_dnodes_dnodes = in.num_shared_dnodes_dnode(context);
	auto shared_dnodes_dnodes = root.initDnodes(num_shared_dnodes_dnodes);
	for(size_t i = 0; i < num_shared_dnodes_dnodes; i++) {
		auto shared_dnodes_dnode = shared_dnodes_dnodes[i];
		auto child_context = in.get_shared_dnodes_dnode(i, context);
		shared_dnodes_dnode.setVal(in.get_dnode_val(child_context));
	}
}
...
inline void write_shared_switches_capnp_type(T &in, ucap::SharedSwitches::Builder &root, Context &context) {
	...
	size_t num_shared_switches_s_switches = in.num_shared_switches_s_switch(context);
	auto shared_switches_s_switches = root.initSSwitches(num_shared_switches_s_switches);
	for(size_t i = 0; i < num_shared_switches_s_switches; i++) {
		auto shared_switches_s_switch = shared_switches_s_switches[i];
		auto child_context = in.get_shared_switches_s_switch(i, context);
		shared_switches_s_switch.setId(in.get_s_switch_id(child_context));
	}
}
...
inline void write_rr_edges_capnp_type(T &in, ucap::RrEdges::Builder &root, Context &context) {
...

	{
		...
		auto rr_edges_shared_dnodes = root.initSharedDnodes();
		write_shared_dnodes_capnp_type(in, rr_edges_shared_dnodes, child_context);
	}

	{
		auto child_context = in.get_rr_edges_shared_switches(context);
		auto rr_edges_shared_switches = root.initSharedSwitches();
		write_shared_switches_capnp_type(in, rr_edges_shared_switches, child_context);
	}
}
