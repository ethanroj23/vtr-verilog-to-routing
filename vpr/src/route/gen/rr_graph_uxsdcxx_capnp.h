...
template <class T, typename Context>
void load_t_node_capnp_type(const ucap::TNode::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack);
template <class T, typename Context>
void load_tile_capnp_type(const ucap::Tile::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack);
template <class T, typename Context>
void load_tiles_capnp_type(const ucap::Tiles::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack);
template <class T, typename Context>
...
template <class T, typename Context>
void load_shared_edges_capnp_type(const ucap::SharedEdges::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack);
template <class T, typename Context>
...
template <class T, typename Context>
inline void write_tile_capnp_type(T &in, ucap::Tile::Builder &root, Context &context);
template <class T, typename Context>
inline void write_tiles_capnp_type(T &in, ucap::Tiles::Builder &root, Context &context);
template <class T, typename Context>
inline void write_shared_edges_capnp_type(T &in, ucap::SharedEdges::Builder &root, Context &context);
template <class T, typename Context>
...
inline void load_rr_nodes_capnp_type(const ucap::RrNodes::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	...
	stack->push_back(std::make_pair("getNode", 0));
	{
		...
		for(const auto & el : data) {
			auto child_context = out.add_rr_nodes_node(context, el.getCapacity(), el.getId(), el.getPtnIdx(), conv_enum_node_type(el.getType(), report_error));
...
template<class T, typename Context>
inline void load_t_node_capnp_type(const ucap::TNode::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	(void)root;
	(void)out;
	(void)context;
	(void)report_error;
...
template<class T, typename Context>
inline void load_tile_capnp_type(const ucap::Tile::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	(void)root;
	(void)out;
	(void)context;
	(void)report_error;
	...
	stack->push_back(std::make_pair("getTNode", 0));
	{
...
template<class T, typename Context>
inline void load_tiles_capnp_type(const ucap::Tiles::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	(void)root;
	(void)out;
	(void)context;
	(void)report_error;
	...
	stack->push_back(std::make_pair("getTile", 0));
	{
		auto data = root.getTiles();
		out.preallocate_tiles_tile(context, data.size());
		for(const auto & el : data) {
			auto child_context = out.add_tiles_tile(context, el.getX(), el.getY());
			load_tile_capnp_type(el, out, child_context, report_error, stack);
...
template<class T, typename Context>
inline void load_shared_edges_capnp_type(const ucap::SharedEdges::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	(void)root;
	(void)out;
	(void)context;
	(void)report_error;
	...
	stack->push_back(std::make_pair("getEdge", 0));
	{
		...
		out.preallocate_shared_edges_edge(context, data.size());
		for(const auto & el : data) {
			auto child_context = out.add_shared_edges_edge(context, el.getDx(), el.getDy(), el.getSwitchId(), el.getTileIdx());
...
inline void load_rr_edges_capnp_type(const ucap::RrEdges::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	...
	stack->push_back(std::make_pair("getSharedEdges", 0));
	{
		auto data = root.getSharedEdgeses();
		out.preallocate_rr_edges_shared_edges(context, data.size());
		for(const auto & el : data) {
			auto child_context = out.add_rr_edges_shared_edges(context, el.getId());
			load_shared_edges_capnp_type(el, out, child_context, report_error, stack);
...
inline void load_rr_graph_capnp_type(const ucap::RrGraph::Reader &root, T &out, Context &context, const std::function<void(const char*)> * report_error, std::vector<std::pair<const char *, size_t>> * stack){
	...
	stack->pop_back();
	stack->push_back(std::make_pair("getTiles", 0));
	if (root.hasTiles()) {
		auto child_el = root.getTiles();
		auto child_context = out.init_rr_graph_tiles(context);
		load_tiles_capnp_type(child_el, out, child_context, report_error, stack);
...
inline void write_tile_capnp_type(T &in, ucap::Tile::Builder &root, Context &context) {
	...
	size_t num_tile_t_nodes = in.num_tile_t_node(context);
	auto tile_t_nodes = root.initTNodes(num_tile_t_nodes);
	for(size_t i = 0; i < num_tile_t_nodes; i++) {
		auto tile_t_node = tile_t_nodes[i];
		auto child_context = in.get_tile_t_node(i, context);
		tile_t_node.setId(in.get_t_node_id(child_context));
	}
}
...
inline void write_tiles_capnp_type(T &in, ucap::Tiles::Builder &root, Context &context) {
	...
	size_t num_tiles_tiles = in.num_tiles_tile(context);
	auto tiles_tiles = root.initTiles(num_tiles_tiles);
	for(size_t i = 0; i < num_tiles_tiles; i++) {
		auto tiles_tile = tiles_tiles[i];
		auto child_context = in.get_tiles_tile(i, context);
		tiles_tile.setX(in.get_tile_x(child_context));
		tiles_tile.setY(in.get_tile_y(child_context));
		write_tile_capnp_type(in, tiles_tile, child_context);
	}
}
...
inline void write_shared_edges_capnp_type(T &in, ucap::SharedEdges::Builder &root, Context &context) {
	...
	size_t num_shared_edges_edges = in.num_shared_edges_edge(context);
	auto shared_edges_edges = root.initEdges(num_shared_edges_edges);
	for(size_t i = 0; i < num_shared_edges_edges; i++) {
		auto shared_edges_edge = shared_edges_edges[i];
		auto child_context = in.get_shared_edges_edge(i, context);
		shared_edges_edge.setDx(in.get_edge_dx(child_context));
		shared_edges_edge.setDy(in.get_edge_dy(child_context));
		shared_edges_edge.setSwitchId(in.get_edge_switch_id(child_context));
		shared_edges_edge.setTileIdx(in.get_edge_tile_idx(child_context));
	}
}
...
inline void write_rr_edges_capnp_type(T &in, ucap::RrEdges::Builder &root, Context &context) {
	...
	size_t num_rr_edges_shared_edgeses = in.num_rr_edges_shared_edges(context);
	auto rr_edges_shared_edgeses = root.initSharedEdgeses(num_rr_edges_shared_edgeses);
	for(size_t i = 0; i < num_rr_edges_shared_edgeses; i++) {
		auto rr_edges_shared_edges = rr_edges_shared_edgeses[i];
		auto child_context = in.get_rr_edges_shared_edges(i, context);
		rr_edges_shared_edges.setId(in.get_shared_edges_id(child_context));
		write_shared_edges_capnp_type(in, rr_edges_shared_edges, child_context);
	}
}
...
inline void write_rr_graph_capnp_type(T &in, ucap::RrGraph::Builder &root, Context &context) {
...

	{
		...
		auto rr_graph_tiles = root.initTiles();
		write_tiles_capnp_type(in, rr_graph_tiles, child_context);
	}
