...
inline void load_node_required_attributes(const pugi::xml_node &root, unsigned int * capacity, unsigned int * id, unsigned int * ptn_idx, enum_node_type * type, const std::function<void(const char*)> * report_error);
template <class T, typename Context>
...
inline void load_edge_required_attributes(const pugi::xml_node &root, int * dx, int * dy, unsigned int * switch_id, unsigned int * tile_idx, const std::function<void(const char*)> * report_error);
template <class T, typename Context>
inline void load_shared_edges(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug);
inline void load_shared_edges_required_attributes(const pugi::xml_node &root, unsigned int * id, const std::function<void(const char*)> * report_error);
template <class T, typename Context>
...
template <class T>
inline void write_tile(T &in, std::ostream &os, const void *data, void *iter);
template <class T>
inline void write_tiles(T &in, std::ostream &os, const void *data, void *iter);
template <class T>
inline void write_shared_edges(T &in, std::ostream &os, const void *data, void *iter);
template <class T>
...
enum class atok_t_node {CAPACITY, DIRECTION, ID, PTN_IDX, TYPE};
constexpr const char *atok_lookup_t_node[] = {"capacity", "direction", "id", "ptn_idx", "type"};

enum class gtok_t_rr_nodes {NODE};
constexpr const char *gtok_lookup_t_rr_nodes[] = {"node"};

enum class atok_t_t_node {ID};
constexpr const char *atok_lookup_t_t_node[] = {"id"};

enum class gtok_t_tile {T_NODE};
constexpr const char *gtok_lookup_t_tile[] = {"t_node"};
enum class atok_t_tile {X, Y};
constexpr const char *atok_lookup_t_tile[] = {"x", "y"};

enum class gtok_t_tiles {TILE};
constexpr const char *gtok_lookup_t_tiles[] = {"tile"};

enum class atok_t_edge {DX, DY, SWITCH_ID, TILE_IDX};
constexpr const char *atok_lookup_t_edge[] = {"dx", "dy", "switch_id", "tile_idx"};

enum class gtok_t_shared_edges {EDGE};
constexpr const char *gtok_lookup_t_shared_edges[] = {"edge"};
enum class atok_t_shared_edges {ID};
constexpr const char *atok_lookup_t_shared_edges[] = {"id"};

enum class gtok_t_rr_edges {SHARED_EDGES};
constexpr const char *gtok_lookup_t_rr_edges[] = {"shared_edges"};
enum class gtok_t_rr_graph {CHANNELS, SWITCHES, SEGMENTS, BLOCK_TYPES, GRID, RR_NODES, TILES, RR_EDGES};
constexpr const char *gtok_lookup_t_rr_graph[] = {"channels", "switches", "segments", "block_types", "grid", "rr_nodes", "tiles", "rr_edges"};
...
inline atok_t_t_node lex_attr_t_t_node(const char *in, const std::function<void(const char *)> * report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 2:
		switch(in[0]){
	...
	noreturn_report(report_error, ("Found unrecognized attribute " + std::string(in) + " of <t_node>.").c_str());
}

inline gtok_t_tile lex_node_t_tile(const char *in, const std::function<void(const char *)> *report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 6:
		switch(*((triehash_uu32*)&in[0])){
		case onechar('t', 0, 32) | onechar('_', 8, 32) | onechar('n', 16, 32) | onechar('o', 24, 32):
			switch(in[4]){
			case onechar('d', 0, 8):
				switch(in[5]){
				case onechar('e', 0, 8):
					return gtok_t_tile::T_NODE;
				break;
				default: break;
				}
			break;
	...
	noreturn_report(report_error, ("Found unrecognized child " + std::string(in) + " of <tile>.").c_str());
}
inline atok_t_tile lex_attr_t_tile(const char *in, const std::function<void(const char *)> * report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 1:
		switch(in[0]){
		case onechar('x', 0, 8):
			return atok_t_tile::X;
		break;
		case onechar('y', 0, 8):
			return atok_t_tile::Y;
		break;
		default: break;
		}
		break;
	...
	noreturn_report(report_error, ("Found unrecognized attribute " + std::string(in) + " of <tile>.").c_str());
}

inline gtok_t_tiles lex_node_t_tiles(const char *in, const std::function<void(const char *)> *report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 4:
		switch(*((triehash_uu32*)&in[0])){
	...
	noreturn_report(report_error, ("Found unrecognized child " + std::string(in) + " of <tiles>.").c_str());
}
...
inline atok_t_edge lex_attr_t_edge(const char *in, const std::function<void(const char *)> * report_error){
	...
	case 2:
		...
		case onechar('d', 0, 8):
			switch(in[1]){
			case onechar('x', 0, 8):
				return atok_t_edge::DX;
			break;
			case onechar('y', 0, 8):
				return atok_t_edge::DY;
			break;
			default: break;
			}
		break;
	...
	case 8:
		switch(*((triehash_uu64*)&in[0])){
		case onechar('t', 0, 64) | onechar('i', 8, 64) | onechar('l', 16, 64) | onechar('e', 24, 64) | onechar('_', 32, 64) | onechar('i', 40, 64) | onechar('d', 48, 64) | onechar('x', 56, 64):
			return atok_t_edge::TILE_IDX;
		break;
		default: break;
		}
		break;
...
inline gtok_t_shared_edges lex_node_t_shared_edges(const char *in, const std::function<void(const char *)> *report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 4:
		switch(*((triehash_uu32*)&in[0])){
		case onechar('e', 0, 32) | onechar('d', 8, 32) | onechar('g', 16, 32) | onechar('e', 24, 32):
			return gtok_t_shared_edges::EDGE;
		break;
		default: break;
		}
		break;
	...
	noreturn_report(report_error, ("Found unrecognized child " + std::string(in) + " of <shared_edges>.").c_str());
}
inline atok_t_shared_edges lex_attr_t_shared_edges(const char *in, const std::function<void(const char *)> * report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 2:
		switch(in[0]){
	...
	noreturn_report(report_error, ("Found unrecognized attribute " + std::string(in) + " of <shared_edges>.").c_str());
}
...
inline gtok_t_rr_edges lex_node_t_rr_edges(const char *in, const std::function<void(const char *)> *report_error){
	...
	case 12:
		switch(*((triehash_uu64*)&in[0])){
		case onechar('s', 0, 64) | onechar('h', 8, 64) | onechar('a', 16, 64) | onechar('r', 24, 64) | onechar('e', 32, 64) | onechar('d', 40, 64) | onechar('_', 48, 64) | onechar('e', 56, 64):
			switch(*((triehash_uu32*)&in[8])){
			case onechar('d', 0, 32) | onechar('g', 8, 32) | onechar('e', 16, 32) | onechar('s', 24, 32):
				return gtok_t_rr_edges::SHARED_EDGES;
			break;
			default: break;
			}
		break;
...
inline void load_node_required_attributes(const pugi::xml_node &root, unsigned int * capacity, unsigned int * id, unsigned int * ptn_idx, enum_node_type * type, const std::function<void(const char *)> * report_error){
	...
	std::bitset<5> test_astate = astate | std::bitset<5>(0b00010);
...
inline void load_t_node_required_attributes(const pugi::xml_node &root, unsigned int * id, const std::function<void(const char *)> * report_error){
	std::bitset<1> astate = 0;
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		atok_t_t_node in = lex_attr_t_t_node(attr.name(), report_error);
		if(astate[(int)in] == 0) astate[(int)in] = 1;
		else noreturn_report(report_error, ("Duplicate attribute " + std::string(attr.name()) + " in <t_node>.").c_str());
		switch(in){
		case atok_t_t_node::ID:
			*id = load_unsigned_int(attr.value(), report_error);
			break;
		default: break; /* Not possible. */
		}
	...
	std::bitset<1> test_astate = astate | std::bitset<1>(0b0);
	if(!test_astate.all()) attr_error(test_astate, atok_lookup_t_t_node, report_error);
}

inline void load_tile_required_attributes(const pugi::xml_node &root, unsigned int * x, unsigned int * y, const std::function<void(const char *)> * report_error){
	std::bitset<2> astate = 0;
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		atok_t_tile in = lex_attr_t_tile(attr.name(), report_error);
		if(astate[(int)in] == 0) astate[(int)in] = 1;
		else noreturn_report(report_error, ("Duplicate attribute " + std::string(attr.name()) + " in <tile>.").c_str());
		switch(in){
		case atok_t_tile::X:
	...
	std::bitset<2> test_astate = astate | std::bitset<2>(0b00);
	if(!test_astate.all()) attr_error(test_astate, atok_lookup_t_tile, report_error);
}
	...
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		...
		case atok_t_edge::SWITCH_ID:
			*switch_id = load_unsigned_int(attr.value(), report_error);
			break;
		case atok_t_edge::TILE_IDX:
			*tile_idx = load_unsigned_int(attr.value(), report_error);
			break;
		default: break; /* Not possible. */
		}
	}
...
inline void load_shared_edges_required_attributes(const pugi::xml_node &root, unsigned int * id, const std::function<void(const char *)> * report_error){
	std::bitset<1> astate = 0;
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		atok_t_shared_edges in = lex_attr_t_shared_edges(attr.name(), report_error);
		if(astate[(int)in] == 0) astate[(int)in] = 1;
		else noreturn_report(report_error, ("Duplicate attribute " + std::string(attr.name()) + " in <shared_edges>.").c_str());
		switch(in){
		case atok_t_shared_edges::ID:
			*id = load_unsigned_int(attr.value(), report_error);
			break;
		default: break; /* Not possible. */
		}
	...
	std::bitset<1> test_astate = astate | std::bitset<1>(0b0);
	if(!test_astate.all()) attr_error(test_astate, atok_lookup_t_shared_edges, report_error);
}
template<class T, typename Context>
inline void load_channel(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	(void)root;
		...
		case gtok_t_rr_nodes::NODE:
			{
				...
				load_node_required_attributes(node, &node_capacity, &node_id, &node_ptn_idx, &node_type, report_error);
				auto child_context = out.add_rr_nodes_node(context, node_capacity, node_id, node_ptn_idx, node_type);
...
inline void load_t_node(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	if(root.first_child().type() == pugi::node_element)
		noreturn_report(report_error, "Unexpected child element in <t_node>.");

}

constexpr int NUM_T_TILE_STATES = 1;
constexpr const int NUM_T_TILE_INPUTS = 1;
constexpr int gstate_t_tile[NUM_T_TILE_STATES][NUM_T_TILE_INPUTS] = {
	{0},
};
template<class T, typename Context>
inline void load_tile(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	// Preallocate arrays by counting child nodes (if any)
	size_t t_node_count = 0;
	{
		int next, state=0;
		for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()) {
			*offset_debug = node.offset_debug();
			gtok_t_tile in = lex_node_t_tile(node.name(), report_error);
			next = gstate_t_tile[state][(int)in];
			if(next == -1)
				dfa_error(gtok_lookup_t_tile[(int)in], gstate_t_tile[state], gtok_lookup_t_tile, 1, report_error);
			state = next;
			switch(in) {
			case gtok_t_tile::T_NODE:
				t_node_count += 1;
				break;
			default: break; /* Not possible. */
			}
		...
		out.preallocate_tile_t_node(context, t_node_count);
	}
	int next, state=0;
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		*offset_debug = node.offset_debug();
	...
	if(state != 0) dfa_error("end of input", gstate_t_tile[state], gtok_lookup_t_tile, 1, report_error);

}

constexpr int NUM_T_TILES_STATES = 2;
constexpr const int NUM_T_TILES_INPUTS = 1;
constexpr int gstate_t_tiles[NUM_T_TILES_STATES][NUM_T_TILES_INPUTS] = {
	{0},
	{0},
};
template<class T, typename Context>
inline void load_tiles(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	if(root.first_attribute())
		noreturn_report(report_error, "Unexpected attribute in <tiles>.");
	...
	size_t tile_count = 0;
	{
		...
		for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()) {
			*offset_debug = node.offset_debug();
			gtok_t_tiles in = lex_node_t_tiles(node.name(), report_error);
			next = gstate_t_tiles[state][(int)in];
			if(next == -1)
				dfa_error(gtok_lookup_t_tiles[(int)in], gstate_t_tiles[state], gtok_lookup_t_tiles, 1, report_error);
			state = next;
			switch(in) {
		...
		out.preallocate_tiles_tile(context, tile_count);
	}
	int next, state=1;
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		*offset_debug = node.offset_debug();
	...
	if(state != 0) dfa_error("end of input", gstate_t_tiles[state], gtok_lookup_t_tiles, 1, report_error);
...
inline void load_edge(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	if(root.first_child().type() == pugi::node_element)
		noreturn_report(report_error, "Unexpected child element in <edge>.");

}

constexpr int NUM_T_SHARED_EDGES_STATES = 1;
constexpr const int NUM_T_SHARED_EDGES_INPUTS = 1;
constexpr int gstate_t_shared_edges[NUM_T_SHARED_EDGES_STATES][NUM_T_SHARED_EDGES_INPUTS] = {
	{0},
};
template<class T, typename Context>
inline void load_shared_edges(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	size_t edge_count = 0;
	{
		...
		for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()) {
			*offset_debug = node.offset_debug();
			gtok_t_shared_edges in = lex_node_t_shared_edges(node.name(), report_error);
			next = gstate_t_shared_edges[state][(int)in];
			if(next == -1)
				dfa_error(gtok_lookup_t_shared_edges[(int)in], gstate_t_shared_edges[state], gtok_lookup_t_shared_edges, 1, report_error);
			state = next;
			switch(in) {
			case gtok_t_shared_edges::EDGE:
				edge_count += 1;
				break;
			default: break; /* Not possible. */
			}
		...
		out.preallocate_shared_edges_edge(context, edge_count);
	}
	int next, state=0;
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		*offset_debug = node.offset_debug();
		...
		case gtok_t_shared_edges::EDGE:
			{
				int edge_dx;
				memset(&edge_dx, 0, sizeof(edge_dx));
				int edge_dy;
				memset(&edge_dy, 0, sizeof(edge_dy));
				unsigned int edge_switch_id;
				memset(&edge_switch_id, 0, sizeof(edge_switch_id));
				unsigned int edge_tile_idx;
				memset(&edge_tile_idx, 0, sizeof(edge_tile_idx));
				load_edge_required_attributes(node, &edge_dx, &edge_dy, &edge_switch_id, &edge_tile_idx, report_error);
				auto child_context = out.add_shared_edges_edge(context, edge_dx, edge_dy, edge_switch_id, edge_tile_idx);
				load_edge(node, out, child_context, report_error, offset_debug);
				out.finish_shared_edges_edge(child_context);
			}
			break;
		default: break; /* Not possible. */
		}
	}
	if(state != 0) dfa_error("end of input", gstate_t_shared_edges[state], gtok_lookup_t_shared_edges, 1, report_error);
	...
	size_t shared_edges_count = 0;
	{
		...
		for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()) {
			*offset_debug = node.offset_debug();
			...
			state = next;
			switch(in) {
			case gtok_t_rr_edges::SHARED_EDGES:
		...
		out.preallocate_rr_edges_shared_edges(context, shared_edges_count);
	}
	int next, state=1;
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		*offset_debug = node.offset_debug();
		...
		case gtok_t_rr_edges::SHARED_EDGES:
			{
				unsigned int shared_edges_id;
				memset(&shared_edges_id, 0, sizeof(shared_edges_id));
				load_shared_edges_required_attributes(node, &shared_edges_id, report_error);
				auto child_context = out.add_rr_edges_shared_edges(context, shared_edges_id);
				load_shared_edges(node, out, child_context, report_error, offset_debug);
				out.finish_rr_edges_shared_edges(child_context);
			}
			break;
		default: break; /* Not possible. */
		}
	...
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		*offset_debug = node.offset_debug();
		...
		case gtok_t_rr_graph::RR_NODES:
			...
			break;
		case gtok_t_rr_graph::TILES:
			{
				auto child_context = out.init_rr_graph_tiles(context);
				load_tiles(node, out, child_context, report_error, offset_debug);
				out.finish_rr_graph_tiles(child_context);
			}
			break;
...
template<class T, typename Context>
inline void write_tile(T &in, std::ostream &os, Context &context){
	(void)in;
	(void)os;
	(void)context;
	{
		for(size_t i=0, n=in.num_tile_t_node(context); i<n; i++){
			auto child_context = in.get_tile_t_node(i, context);
			os << "<t_node";
			os << " id=\"" << in.get_t_node_id(child_context) << "\"";
			os << "/>\n";
		}
	}
}
...
template<class T, typename Context>
inline void write_tiles(T &in, std::ostream &os, Context &context){
	(void)in;
	(void)os;
	(void)context;
	{
		for(size_t i=0, n=in.num_tiles_tile(context); i<n; i++){
			auto child_context = in.get_tiles_tile(i, context);
			os << "<tile";
			os << " x=\"" << in.get_tile_x(child_context) << "\"";
			os << " y=\"" << in.get_tile_y(child_context) << "\"";
			os << ">";
			write_tile(in, os, child_context);
			os << "</tile>\n";
		}
	}
}
...
template<class T, typename Context>
inline void write_shared_edges(T &in, std::ostream &os, Context &context){
	(void)in;
	(void)os;
	(void)context;
	{
		for(size_t i=0, n=in.num_shared_edges_edge(context); i<n; i++){
			auto child_context = in.get_shared_edges_edge(i, context);
			os << "<edge";
			os << " dx=\"" << in.get_edge_dx(child_context) << "\"";
			os << " dy=\"" << in.get_edge_dy(child_context) << "\"";
			os << " switch_id=\"" << in.get_edge_switch_id(child_context) << "\"";
			os << " tile_idx=\"" << in.get_edge_tile_idx(child_context) << "\"";
			os << "/>\n";
		}
	}
}
...
inline void write_rr_edges(T &in, std::ostream &os, Context &context){
	...
	(void)context;
	{
		for(size_t i=0, n=in.num_rr_edges_shared_edges(context); i<n; i++){
			auto child_context = in.get_rr_edges_shared_edges(i, context);
			os << "<shared_edges";
			os << " id=\"" << in.get_shared_edges_id(child_context) << "\"";
			os << ">";
			write_shared_edges(in, os, child_context);
			os << "</shared_edges>\n";
		}
	}
}
...
inline void write_rr_graph(T &in, std::ostream &os, Context &context){
	...
	{
		auto child_context = in.get_rr_graph_tiles(context);
		os << "<tiles>\n";
		write_tiles(in, os, child_context);
		os << "</tiles>\n";
	}
	{
