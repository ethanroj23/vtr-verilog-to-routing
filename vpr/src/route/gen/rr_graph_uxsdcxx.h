 ...
 * md5sum of input file: af77dfa3aea913b74b8daaa8398c52ea
 */
...
template <class T, typename Context>
inline void load_rr_switch(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug);
inline void load_rr_switch_required_attributes(const pugi::xml_node &root, unsigned int * id, const std::function<void(const char*)> * report_error);
template <class T, typename Context>
inline void load_rr_switches(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug);
template <class T, typename Context>
...
inline void load_node_required_attributes(const pugi::xml_node &root, unsigned int * capacity, unsigned int * id, unsigned int * s_idx, enum_node_type * type, const std::function<void(const char*)> * report_error);
template <class T, typename Context>
...
inline void load_edge_required_attributes(const pugi::xml_node &root, unsigned int * sink_node, unsigned int * src_node, const std::function<void(const char*)> * report_error);
template <class T, typename Context>
...
template <class T>
inline void write_rr_switches(T &in, std::ostream &os, const void *data, void *iter);
template <class T>
...
enum class atok_t_rr_switch {ID};
constexpr const char *atok_lookup_t_rr_switch[] = {"id"};

enum class gtok_t_rr_switches {RR_SWITCH};
constexpr const char *gtok_lookup_t_rr_switches[] = {"rr_switch"};
...
enum class atok_t_node {CAPACITY, DIRECTION, ID, S_IDX, TYPE};
constexpr const char *atok_lookup_t_node[] = {"capacity", "direction", "id", "s_idx", "type"};
...
enum class atok_t_edge {SINK_NODE, SRC_NODE};
constexpr const char *atok_lookup_t_edge[] = {"sink_node", "src_node"};

enum class gtok_t_rr_edges {EDGE};
constexpr const char *gtok_lookup_t_rr_edges[] = {"edge"};
enum class gtok_t_rr_graph {CHANNELS, SWITCHES, SEGMENTS, BLOCK_TYPES, GRID, RR_SWITCHES, RR_NODES, RR_EDGES};
constexpr const char *gtok_lookup_t_rr_graph[] = {"channels", "switches", "segments", "block_types", "grid", "rr_switches", "rr_nodes", "rr_edges"};
...
inline atok_t_rr_switch lex_attr_t_rr_switch(const char *in, const std::function<void(const char *)> * report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 2:
		switch(in[0]){
	...
	noreturn_report(report_error, ("Found unrecognized attribute " + std::string(in) + " of <rr_switch>.").c_str());
}

inline gtok_t_rr_switches lex_node_t_rr_switches(const char *in, const std::function<void(const char *)> *report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 9:
		switch(*((triehash_uu64*)&in[0])){
		case onechar('r', 0, 64) | onechar('r', 8, 64) | onechar('_', 16, 64) | onechar('s', 24, 64) | onechar('w', 32, 64) | onechar('i', 40, 64) | onechar('t', 48, 64) | onechar('c', 56, 64):
			switch(in[8]){
			case onechar('h', 0, 8):
				return gtok_t_rr_switches::RR_SWITCH;
			break;
			default: break;
			}
		break;
	...
	noreturn_report(report_error, ("Found unrecognized child " + std::string(in) + " of <rr_switches>.").c_str());
}
		...
		case onechar('r', 0, 64) | onechar('r', 8, 64) | onechar('_', 16, 64) | onechar('s', 24, 64) | onechar('w', 32, 64) | onechar('i', 40, 64) | onechar('t', 48, 64) | onechar('c', 56, 64):
			...
			case onechar('h', 0, 8):
				...
				case onechar('e', 0, 8):
					switch(in[10]){
					case onechar('s', 0, 8):
						return gtok_t_rr_graph::RR_SWITCHES;
					break;
					default: break;
					}
				break;
...
inline void load_rr_switch_required_attributes(const pugi::xml_node &root, unsigned int * id, const std::function<void(const char *)> * report_error){
	std::bitset<1> astate = 0;
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		atok_t_rr_switch in = lex_attr_t_rr_switch(attr.name(), report_error);
		if(astate[(int)in] == 0) astate[(int)in] = 1;
		else noreturn_report(report_error, ("Duplicate attribute " + std::string(attr.name()) + " in <rr_switch>.").c_str());
		switch(in){
		case atok_t_rr_switch::ID:
	...
	std::bitset<1> test_astate = astate | std::bitset<1>(0b0);
	if(!test_astate.all()) attr_error(test_astate, atok_lookup_t_rr_switch, report_error);
}
...
inline void load_rr_switch(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	if(root.first_child().type() == pugi::node_element)
		noreturn_report(report_error, "Unexpected child element in <rr_switch>.");

}

constexpr int NUM_T_RR_SWITCHES_STATES = 2;
constexpr const int NUM_T_RR_SWITCHES_INPUTS = 1;
constexpr int gstate_t_rr_switches[NUM_T_RR_SWITCHES_STATES][NUM_T_RR_SWITCHES_INPUTS] = {
	{0},
	{0},
};
template<class T, typename Context>
inline void load_rr_switches(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	if(root.first_attribute())
		noreturn_report(report_error, "Unexpected attribute in <rr_switches>.");

	// Preallocate arrays by counting child nodes (if any)
	size_t rr_switch_count = 0;
	{
		int next, state=1;
		for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()) {
			*offset_debug = node.offset_debug();
			gtok_t_rr_switches in = lex_node_t_rr_switches(node.name(), report_error);
			next = gstate_t_rr_switches[state][(int)in];
			if(next == -1)
				dfa_error(gtok_lookup_t_rr_switches[(int)in], gstate_t_rr_switches[state], gtok_lookup_t_rr_switches, 1, report_error);
			state = next;
			switch(in) {
			case gtok_t_rr_switches::RR_SWITCH:
				rr_switch_count += 1;
				break;
			default: break; /* Not possible. */
			}
		...
		out.preallocate_rr_switches_rr_switch(context, rr_switch_count);
	}
	int next, state=1;
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		*offset_debug = node.offset_debug();
		...
		case gtok_t_rr_switches::RR_SWITCH:
			{
				unsigned int rr_switch_id;
				memset(&rr_switch_id, 0, sizeof(rr_switch_id));
				load_rr_switch_required_attributes(node, &rr_switch_id, report_error);
				auto child_context = out.add_rr_switches_rr_switch(context, rr_switch_id);
				load_rr_switch(node, out, child_context, report_error, offset_debug);
				out.finish_rr_switches_rr_switch(child_context);
			}
			break;
		default: break; /* Not possible. */
		}
	}
	if(state != 0) dfa_error("end of input", gstate_t_rr_switches[state], gtok_lookup_t_rr_switches, 1, report_error);
		...
		case gtok_t_rr_nodes::NODE:
			{
				...
				load_node_required_attributes(node, &node_capacity, &node_id, &node_s_idx, &node_type, report_error);
				auto child_context = out.add_rr_nodes_node(context, node_capacity, node_id, node_s_idx, node_type);
		...
		case gtok_t_rr_edges::EDGE:
			{
				...
				load_edge_required_attributes(node, &edge_sink_node, &edge_src_node, report_error);
				auto child_context = out.add_rr_edges_edge(context, edge_sink_node, edge_src_node);
	...
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		*offset_debug = node.offset_debug();
		...
		case gtok_t_rr_graph::GRID:
			...
			break;
		case gtok_t_rr_graph::RR_SWITCHES:
			{
				auto child_context = out.init_rr_graph_rr_switches(context);
				load_rr_switches(node, out, child_context, report_error, offset_debug);
				out.finish_rr_graph_rr_switches(child_context);
			}
			break;
...
template<class T, typename Context>
inline void write_rr_switches(T &in, std::ostream &os, Context &context){
	(void)in;
	(void)os;
	(void)context;
	{
		for(size_t i=0, n=in.num_rr_switches_rr_switch(context); i<n; i++){
			auto child_context = in.get_rr_switches_rr_switch(i, context);
			os << "<rr_switch";
			os << " id=\"" << in.get_rr_switch_id(child_context) << "\"";
			os << "/>\n";
		}
	}
}
...
inline void write_rr_graph(T &in, std::ostream &os, Context &context){
	...
	{
		auto child_context = in.get_rr_graph_rr_switches(context);
		os << "<rr_switches>\n";
		write_rr_switches(in, os, child_context);
		os << "</rr_switches>\n";
	}
	{
