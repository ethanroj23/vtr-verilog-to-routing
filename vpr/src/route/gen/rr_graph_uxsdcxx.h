...
inline void load_edge_required_attributes(const pugi::xml_node& root, int* dnode, unsigned int* switch_id, const std::function<void(const char*)>* report_error);
template<class T, typename Context>
...
enum class atok_t_edge { DNODE,
                         SWITCH_ID };
constexpr const char* atok_lookup_t_edge[] = {"dnode", "switch_id"};
...
inline void load_edge_required_attributes(const pugi::xml_node& root, int* dnode, unsigned int* switch_id, const std::function<void(const char*)>* report_error) {
    ...
    for (pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()) {
        ...
        switch (in) {
            case atok_t_edge::DNODE:
                *dnode = load_int(attr.value(), report_error);
                break;
...
constexpr int NUM_T_RR_EDGES_STATES = 1;
    ...
    for (pugi::xml_node node = root.first_child(); node; node = node.next_sibling()) {
        *offset_debug = node.offset_debug();
        ...
        switch (in) {
            case gtok_t_rr_edges::EDGE: {
                int edge_dnode;
                memset(&edge_dnode, 0, sizeof(edge_dnode));
                unsigned int edge_switch_id;
                memset(&edge_switch_id, 0, sizeof(edge_switch_id));
                load_edge_required_attributes(node, &edge_dnode, &edge_switch_id, report_error);
                auto child_context = out.add_rr_edges_edge(context, edge_dnode, edge_switch_id);
...
inline void write_rr_edges(T& in, std::ostream& os, Context& context) {
    ...
    (void)context;
    {
        for (size_t i = 0, n = in.num_rr_edges_edge(context); i < n; i++) {
            ...
            os << "<edge";
            os << " dnode=\"" << in.get_edge_dnode(child_context) << "\"";
