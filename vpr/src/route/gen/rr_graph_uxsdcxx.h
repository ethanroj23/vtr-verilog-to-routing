...
inline void load_node_required_attributes(const pugi::xml_node &root, unsigned int * capacity, unsigned int * d_idx, unsigned int * id, unsigned int * s_idx, enum_node_type * type, const std::function<void(const char*)> * report_error);
template <class T, typename Context>
...
template <class T, typename Context>
inline void load_dnode(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug);
inline void load_dnode_required_attributes(const pugi::xml_node &root, int * val, const std::function<void(const char*)> * report_error);
template <class T, typename Context>
inline void load_shared_dnodes(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug);
template <class T, typename Context>
inline void load_s_switch(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug);
inline void load_s_switch_required_attributes(const pugi::xml_node &root, unsigned int * id, const std::function<void(const char*)> * report_error);
template <class T, typename Context>
inline void load_shared_switches(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug);
template <class T, typename Context>
...
template <class T>
inline void write_shared_dnodes(T &in, std::ostream &os, const void *data, void *iter);
template <class T>
inline void write_shared_switches(T &in, std::ostream &os, const void *data, void *iter);
template <class T>
...
enum class atok_t_node {CAPACITY, D_IDX, DIRECTION, ID, S_IDX, TYPE};
constexpr const char *atok_lookup_t_node[] = {"capacity", "d_idx", "direction", "id", "s_idx", "type"};

enum class gtok_t_rr_nodes {NODE};
constexpr const char *gtok_lookup_t_rr_nodes[] = {"node"};

enum class atok_t_dnode {VAL};
constexpr const char *atok_lookup_t_dnode[] = {"val"};

enum class gtok_t_shared_dnodes {DNODE};
constexpr const char *gtok_lookup_t_shared_dnodes[] = {"dnode"};

enum class atok_t_s_switch {ID};
constexpr const char *atok_lookup_t_s_switch[] = {"id"};

enum class gtok_t_shared_switches {S_SWITCH};
constexpr const char *gtok_lookup_t_shared_switches[] = {"s_switch"};
enum class gtok_t_rr_edges {SHARED_DNODES, SHARED_SWITCHES};
constexpr const char *gtok_lookup_t_rr_edges[] = {"shared_dnodes", "shared_switches"};
...
inline atok_t_node lex_attr_t_node(const char *in, const std::function<void(const char *)> * report_error){
	...
	case 5:
		switch(*((triehash_uu32*)&in[0])){
		case onechar('d', 0, 32) | onechar('_', 8, 32) | onechar('i', 16, 32) | onechar('d', 24, 32):
			switch(in[4]){
			case onechar('x', 0, 8):
				return atok_t_node::D_IDX;
			break;
			default: break;
			}
		break;
		case onechar('s', 0, 32) | onechar('_', 8, 32) | onechar('i', 16, 32) | onechar('d', 24, 32):
			switch(in[4]){
			case onechar('x', 0, 8):
				return atok_t_node::S_IDX;
			break;
			default: break;
			}
		break;
...
inline atok_t_dnode lex_attr_t_dnode(const char *in, const std::function<void(const char *)> * report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 3:
		switch(in[0]){
	...
	noreturn_report(report_error, ("Found unrecognized attribute " + std::string(in) + " of <dnode>.").c_str());
}

inline gtok_t_shared_dnodes lex_node_t_shared_dnodes(const char *in, const std::function<void(const char *)> *report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 5:
		switch(*((triehash_uu32*)&in[0])){
		case onechar('d', 0, 32) | onechar('n', 8, 32) | onechar('o', 16, 32) | onechar('d', 24, 32):
			switch(in[4]){
			case onechar('e', 0, 8):
				return gtok_t_shared_dnodes::DNODE;
			break;
			default: break;
			}
		break;
	...
	noreturn_report(report_error, ("Found unrecognized child " + std::string(in) + " of <shared_dnodes>.").c_str());
}

inline atok_t_s_switch lex_attr_t_s_switch(const char *in, const std::function<void(const char *)> * report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 2:
		switch(in[0]){
	...
	noreturn_report(report_error, ("Found unrecognized attribute " + std::string(in) + " of <s_switch>.").c_str());
}

inline gtok_t_shared_switches lex_node_t_shared_switches(const char *in, const std::function<void(const char *)> *report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 8:
		switch(*((triehash_uu64*)&in[0])){
		case onechar('s', 0, 64) | onechar('_', 8, 64) | onechar('s', 16, 64) | onechar('w', 24, 64) | onechar('i', 32, 64) | onechar('t', 40, 64) | onechar('c', 48, 64) | onechar('h', 56, 64):
			return gtok_t_shared_switches::S_SWITCH;
		break;
		default: break;
		}
		break;
	...
	noreturn_report(report_error, ("Found unrecognized child " + std::string(in) + " of <shared_switches>.").c_str());
}
...
inline gtok_t_rr_edges lex_node_t_rr_edges(const char *in, const std::function<void(const char *)> *report_error){
	...
	case 13:
		switch(*((triehash_uu64*)&in[0])){
		case onechar('s', 0, 64) | onechar('h', 8, 64) | onechar('a', 16, 64) | onechar('r', 24, 64) | onechar('e', 32, 64) | onechar('d', 40, 64) | onechar('_', 48, 64) | onechar('d', 56, 64):
			switch(*((triehash_uu32*)&in[8])){
			case onechar('n', 0, 32) | onechar('o', 8, 32) | onechar('d', 16, 32) | onechar('e', 24, 32):
				switch(in[12]){
				case onechar('s', 0, 8):
					return gtok_t_rr_edges::SHARED_DNODES;
				break;
				default: break;
				}
			break;
	...
	case 15:
		switch(*((triehash_uu64*)&in[0])){
		case onechar('s', 0, 64) | onechar('h', 8, 64) | onechar('a', 16, 64) | onechar('r', 24, 64) | onechar('e', 32, 64) | onechar('d', 40, 64) | onechar('_', 48, 64) | onechar('s', 56, 64):
			switch(*((triehash_uu32*)&in[8])){
			case onechar('w', 0, 32) | onechar('i', 8, 32) | onechar('t', 16, 32) | onechar('c', 24, 32):
				switch(in[12]){
				case onechar('h', 0, 8):
					switch(in[13]){
					case onechar('e', 0, 8):
						switch(in[14]){
						case onechar('s', 0, 8):
							return gtok_t_rr_edges::SHARED_SWITCHES;
						break;
						default: break;
						}
					break;
	...
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		...
		case atok_t_node::CAPACITY:
			*capacity = load_unsigned_int(attr.value(), report_error);
			break;
		case atok_t_node::D_IDX:
			*d_idx = load_unsigned_int(attr.value(), report_error);
			break;
		...
		case atok_t_node::ID:
			*id = load_unsigned_int(attr.value(), report_error);
			break;
		case atok_t_node::S_IDX:
			*s_idx = load_unsigned_int(attr.value(), report_error);
			break;
...
inline void load_dnode_required_attributes(const pugi::xml_node &root, int * val, const std::function<void(const char *)> * report_error){
	std::bitset<1> astate = 0;
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		atok_t_dnode in = lex_attr_t_dnode(attr.name(), report_error);
		if(astate[(int)in] == 0) astate[(int)in] = 1;
		else noreturn_report(report_error, ("Duplicate attribute " + std::string(attr.name()) + " in <dnode>.").c_str());
		switch(in){
		case atok_t_dnode::VAL:
			*val = load_int(attr.value(), report_error);
			break;
		default: break; /* Not possible. */
		}
	}
	std::bitset<1> test_astate = astate | std::bitset<1>(0b0);
	if(!test_astate.all()) attr_error(test_astate, atok_lookup_t_dnode, report_error);
}

inline void load_s_switch_required_attributes(const pugi::xml_node &root, unsigned int * id, const std::function<void(const char *)> * report_error){
	std::bitset<1> astate = 0;
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		atok_t_s_switch in = lex_attr_t_s_switch(attr.name(), report_error);
		if(astate[(int)in] == 0) astate[(int)in] = 1;
		else noreturn_report(report_error, ("Duplicate attribute " + std::string(attr.name()) + " in <s_switch>.").c_str());
		switch(in){
		case atok_t_s_switch::ID:
			*id = load_unsigned_int(attr.value(), report_error);
			break;
		default: break; /* Not possible. */
		}
	...
	std::bitset<1> test_astate = astate | std::bitset<1>(0b0);
	if(!test_astate.all()) attr_error(test_astate, atok_lookup_t_s_switch, report_error);
}
template<class T, typename Context>
inline void load_channel(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	(void)root;
	...
	*offset_debug = root.offset_debug();
	...
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		...
		case atok_t_node::CAPACITY:
			...
			break;
		case atok_t_node::D_IDX:
			/* Attribute d_idx is already set */
			break;
		case atok_t_node::DIRECTION:
		...
		case atok_t_node::ID:
			...
			break;
		case atok_t_node::S_IDX:
			/* Attribute s_idx is already set */
			break;
		case atok_t_node::TYPE:
		...
		case gtok_t_rr_nodes::NODE:
			{
				...
				unsigned int node_d_idx;
				memset(&node_d_idx, 0, sizeof(node_d_idx));
				unsigned int node_id;
				memset(&node_id, 0, sizeof(node_id));
				unsigned int node_s_idx;
				memset(&node_s_idx, 0, sizeof(node_s_idx));
				enum_node_type node_type;
				memset(&node_type, 0, sizeof(node_type));
				load_node_required_attributes(node, &node_capacity, &node_d_idx, &node_id, &node_s_idx, &node_type, report_error);
				auto child_context = out.add_rr_nodes_node(context, node_capacity, node_d_idx, node_id, node_s_idx, node_type);
...
inline void load_dnode(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	if(root.first_child().type() == pugi::node_element)
		noreturn_report(report_error, "Unexpected child element in <dnode>.");

}

constexpr int NUM_T_SHARED_DNODES_STATES = 1;
constexpr const int NUM_T_SHARED_DNODES_INPUTS = 1;
constexpr int gstate_t_shared_dnodes[NUM_T_SHARED_DNODES_STATES][NUM_T_SHARED_DNODES_INPUTS] = {
	{0},
};
template<class T, typename Context>
inline void load_shared_dnodes(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	if(root.first_attribute())
		noreturn_report(report_error, "Unexpected attribute in <shared_dnodes>.");

	// Preallocate arrays by counting child nodes (if any)
	size_t dnode_count = 0;
	{
		int next, state=0;
		for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()) {
			*offset_debug = node.offset_debug();
			gtok_t_shared_dnodes in = lex_node_t_shared_dnodes(node.name(), report_error);
			next = gstate_t_shared_dnodes[state][(int)in];
			if(next == -1)
				dfa_error(gtok_lookup_t_shared_dnodes[(int)in], gstate_t_shared_dnodes[state], gtok_lookup_t_shared_dnodes, 1, report_error);
			state = next;
			switch(in) {
			case gtok_t_shared_dnodes::DNODE:
				dnode_count += 1;
				break;
			default: break; /* Not possible. */
			}
		...
		out.preallocate_shared_dnodes_dnode(context, dnode_count);
	}
	int next, state=0;
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		*offset_debug = node.offset_debug();
		...
		case gtok_t_shared_dnodes::DNODE:
			{
				int dnode_val;
				memset(&dnode_val, 0, sizeof(dnode_val));
				load_dnode_required_attributes(node, &dnode_val, report_error);
				auto child_context = out.add_shared_dnodes_dnode(context, dnode_val);
				load_dnode(node, out, child_context, report_error, offset_debug);
				out.finish_shared_dnodes_dnode(child_context);
			}
			break;
		default: break; /* Not possible. */
		}
	}
	if(state != 0) dfa_error("end of input", gstate_t_shared_dnodes[state], gtok_lookup_t_shared_dnodes, 1, report_error);
...
inline void load_s_switch(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	if(root.first_child().type() == pugi::node_element)
		noreturn_report(report_error, "Unexpected child element in <s_switch>.");

}

constexpr int NUM_T_SHARED_SWITCHES_STATES = 1;
constexpr const int NUM_T_SHARED_SWITCHES_INPUTS = 1;
constexpr int gstate_t_shared_switches[NUM_T_SHARED_SWITCHES_STATES][NUM_T_SHARED_SWITCHES_INPUTS] = {
	{0},
};
template<class T, typename Context>
inline void load_shared_switches(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	if(root.first_attribute())
		noreturn_report(report_error, "Unexpected attribute in <shared_switches>.");

	// Preallocate arrays by counting child nodes (if any)
	size_t s_switch_count = 0;
	{
		int next, state=0;
		for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()) {
			*offset_debug = node.offset_debug();
			gtok_t_shared_switches in = lex_node_t_shared_switches(node.name(), report_error);
			next = gstate_t_shared_switches[state][(int)in];
			if(next == -1)
				dfa_error(gtok_lookup_t_shared_switches[(int)in], gstate_t_shared_switches[state], gtok_lookup_t_shared_switches, 1, report_error);
			state = next;
			switch(in) {
			case gtok_t_shared_switches::S_SWITCH:
				s_switch_count += 1;
				break;
			default: break; /* Not possible. */
			}
		...
		out.preallocate_shared_switches_s_switch(context, s_switch_count);
	}
	int next, state=0;
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		*offset_debug = node.offset_debug();
		...
		case gtok_t_shared_switches::S_SWITCH:
			{
				unsigned int s_switch_id;
				memset(&s_switch_id, 0, sizeof(s_switch_id));
				load_s_switch_required_attributes(node, &s_switch_id, report_error);
				auto child_context = out.add_shared_switches_s_switch(context, s_switch_id);
				load_s_switch(node, out, child_context, report_error, offset_debug);
				out.finish_shared_switches_s_switch(child_context);
			}
			break;
		default: break; /* Not possible. */
		}
	}
	if(state != 0) dfa_error("end of input", gstate_t_shared_switches[state], gtok_lookup_t_shared_switches, 1, report_error);
	...
	*offset_debug = root.offset_debug();
	...
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		*offset_debug = node.offset_debug();
		...
		if(gstate[(int)in] == 0) gstate[(int)in] = 1;
		else noreturn_report(report_error, ("Duplicate element " + std::string(node.name()) + " in <rr_edges>.").c_str());
		switch(in){
		case gtok_t_rr_edges::SHARED_DNODES:
			{
				auto child_context = out.init_rr_edges_shared_dnodes(context);
				load_shared_dnodes(node, out, child_context, report_error, offset_debug);
				out.finish_rr_edges_shared_dnodes(child_context);
			}
			break;
		case gtok_t_rr_edges::SHARED_SWITCHES:
			{
				auto child_context = out.init_rr_edges_shared_switches(context);
				load_shared_switches(node, out, child_context, report_error, offset_debug);
				out.finish_rr_edges_shared_switches(child_context);
			}
			break;
		default: break; /* Not possible. */
		}
	...
	if(!test_gstate.all()) all_error(test_gstate, gtok_lookup_t_rr_edges, report_error);
...
inline void write_rr_nodes(T &in, std::ostream &os, Context &context){
	...
	(void)context;
	{
		for(size_t i=0, n=in.num_rr_nodes_node(context); i<n; i++){
			...
			os << " d_idx=\"" << in.get_node_d_idx(child_context) << "\"";
			...
			os << " s_idx=\"" << in.get_node_s_idx(child_context) << "\"";
...
template<class T, typename Context>
inline void write_shared_dnodes(T &in, std::ostream &os, Context &context){
	(void)in;
	(void)os;
	(void)context;
	{
		for(size_t i=0, n=in.num_shared_dnodes_dnode(context); i<n; i++){
			auto child_context = in.get_shared_dnodes_dnode(i, context);
			os << "<dnode";
			os << " val=\"" << in.get_dnode_val(child_context) << "\"";
			os << "/>\n";
		}
	}
}
...
template<class T, typename Context>
inline void write_shared_switches(T &in, std::ostream &os, Context &context){
	(void)in;
	(void)os;
	(void)context;
	{
		for(size_t i=0, n=in.num_shared_switches_s_switch(context); i<n; i++){
			auto child_context = in.get_shared_switches_s_switch(i, context);
			os << "<s_switch";
			os << " id=\"" << in.get_s_switch_id(child_context) << "\"";
			os << "/>\n";
		}
	}
}
...
inline void write_rr_edges(T &in, std::ostream &os, Context &context){
	...
	(void)context;
	{
		auto child_context = in.get_rr_edges_shared_dnodes(context);
		os << "<shared_dnodes>\n";
		write_shared_dnodes(in, os, child_context);
		os << "</shared_dnodes>\n";
	}
	{
		auto child_context = in.get_rr_edges_shared_switches(context);
		os << "<shared_switches>\n";
		write_shared_switches(in, os, child_context);
		os << "</shared_switches>\n";
	}
}
