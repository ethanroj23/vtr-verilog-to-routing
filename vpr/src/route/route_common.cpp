...
static t_trace_branch traceback_branch(int node, int target_net_pin_index, std::unordered_set<int>& trace_nodes) {
    ...
    auto rr_type = device_ctx.rr_graph.node_type(RRNodeId(node)) /*ESR API*/;
    if (rr_type != SINK) {
...
static std::pair<t_trace*, t_trace*> add_trace_non_configurable_recurr(int node, std::unordered_set<int>& trace_nodes, int depth) {
    ...
    for (auto iedge : device_ctx.rr_graph.node_non_configurable_edges(RRNodeId(node))) {
        ...
        int to_node = device_ctx.rr_graph.node_edge_sink_node(RREdgeId(iedge)) /*ESR API*/;
                ...
                while (tptr != nullptr) {
                    ...
                    if ((ilow != device_ctx.rr_graph.node_xhigh(RRNodeId(inode)) /*ESR API*/)
                        ...
                        fprintf(fp, "to (%d,%d) ", device_ctx.rr_graph.node_xhigh(RRNodeId(inode)) /*ESR API*/,
                                device_ctx.rr_graph.node_yhigh(RRNodeId(inode)) /*ESR API*/);
    ...
    for (auto blk_id : cluster_ctx.clb_nlist.blocks()) {
        ...
        for (iclass = 0; iclass < (int)type->class_inf.size(); iclass++) {
            ...
            for (iconn = 0; iconn < num_edges; iconn++) {
                to_node = device_ctx.rr_graph.node_edge_sink_node(RREdgeId(iconn)) /*ESR API*/;

                VTR_ASSERT(device_ctx.rr_graph.node_type(RRNodeId(to_node)) /*ESR API*/ == OPIN);
            ...
            for (ipin = 0; ipin < num_local_opin; ipin++) {
                ...
                VTR_ASSERT(device_ctx.rr_graph.node_type(RRNodeId(inode)) /*ESR API*/ == OPIN);
...
void print_traceback(const t_trace* trace) {
    ...
    while (trace) {
        ...
        VTR_LOG("%d (%s)", inode, rr_node_typename[device_ctx.rr_graph.node_type(RRNodeId(inode)) /*ESR API*/]);
        ...
        if (trace->iswitch == OPEN) { //End of a branch
            ...
            for (t_edge_size iedge = 0; iedge < device_ctx.rr_graph.node_num_edges(RRNodeId(trace->index)) /*ESR API*/; ++iedge) {
                ...
                if (to_node == next->index) {
                    ...
                    int rr_iswitch = device_ctx.rr_graph.node_edge_switch(RREdgeId(iedge)) /*ESR API*/;
