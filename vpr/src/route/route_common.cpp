...
/* Returns a vector from [0..device_ctx.rr_graph.size()-1] containing the set
...
std::vector<std::set<ClusterNetId>> collect_rr_node_nets() {
    ...
    std::vector<std::set<ClusterNetId>> rr_node_nets(device_ctx.rr_graph.size());
...
static t_trace_branch traceback_branch(int node, int target_net_pin_index, std::unordered_set<int>& trace_nodes) {
    ...
    while (inode != NO_PREVIOUS) {
        ...
        prev_ptr->iswitch = device_ctx.rr_graph.edge_switch(iedge);
...
static std::pair<t_trace*, t_trace*> add_trace_non_configurable_recurr(int node, std::unordered_set<int>& trace_nodes, int depth) {
    ...
    std::vector<t_edge_with_id> unvisited_non_configurable_edges;
    auto& device_ctx = g_vpr_ctx.device();
    const auto& rr_graph = device_ctx.rr_graph;
    std::vector<t_edge_with_id> edges;
    rr_graph.non_configurable_edge_with_id_range_direct(RRNodeId(node), edges);
    for (auto edge : edges) {
...
void alloc_and_load_rr_node_route_structs() {
    ...
    route_ctx.rr_node_route_inf.resize(device_ctx.rr_graph.size());
    route_ctx.non_configurable_bitset.resize(device_ctx.rr_graph.size());
...
void reset_rr_node_route_structs() {
    ...
    VTR_ASSERT(route_ctx.rr_node_route_inf.size() == size_t(device_ctx.rr_graph.size()));
        ...
        for (auto blk_id : cluster_ctx.clb_nlist.blocks()) {
            ...
            for (iclass = 0; iclass < (int)type->class_inf.size(); iclass++) {
                ...
                for (ipin = 0; ipin < num_local_opin; ipin++) {
                    ...
                    VTR_ASSERT(inode >= 0 && inode < (ssize_t)device_ctx.rr_graph.size());
