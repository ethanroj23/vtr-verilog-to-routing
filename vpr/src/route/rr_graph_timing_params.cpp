...
void add_rr_graph_C_from_switches(float C_ipin_cblock) {
    ...
    for (size_t inode = 0; inode < device_ctx.rr_nodes.size(); inode++) {
        ...
        rr_node_C[inode] += device_ctx.rr_graph.node_C(RRNodeId(inode));
    ...
    for (size_t inode = 0; inode < device_ctx.rr_nodes.size(); inode++) {
        mutable_device_ctx.rr_nodes[inode].set_rc_index(find_create_rr_rc_data(device_ctx.rr_graph.node_R(RRNodeId(inode)), rr_node_C[inode]));
    }
