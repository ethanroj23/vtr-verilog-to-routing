...
void check_route(enum e_route_type route_type, e_check_route_option check_route_option) {
    ...
    auto connected_to_route = std::make_unique<bool[]>(device_ctx.rr_graph.size());
    std::fill_n(connected_to_route.get(), device_ctx.rr_graph.size(), false);
...
static bool check_adjacent(int from_node, int to_node) {
    ...
    reached = false;
    t_edge_soa edges = rr_graph.edge_range_soa(RRNodeId(from_node));
    size_t num_edges = edges.dests.size();
    for (size_t k=0; k < num_edges; k++) {
        if (size_t(edges.dests[k]) == size_t(to_node)) {
            reached = true;
            break;
        }
    }
...
static void check_node_and_range(int inode, enum e_route_type route_type) {
    ...
    if (inode < 0 || inode >= (int)device_ctx.rr_graph.size()) {
        VPR_FATAL_ERROR(VPR_ERROR_ROUTE,
                        "in check_node_and_range: rr_node #%d is out of legal, range (0 to %d).\n", inode, device_ctx.rr_graph.size() - 1);
    }
