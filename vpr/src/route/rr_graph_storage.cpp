...
short t_rr_graph_storage::node_pin_num(RRNodeId id) const {
    auto node_type = node_to_ptn_[id].type_;
    if (node_type != IPIN && node_type != OPIN) {
        VPR_FATAL_ERROR(VPR_ERROR_ROUTE, "Attempted to access RR node 'pin_num' for non-IPIN/OPIN type '%s'", rr_node_typename[node_type]);
    }
    return node_ptc_[id].ptc_.pin_num;
...
void t_rr_graph_storage::set_node_type(RRNodeId id, t_rr_type new_type) {
    ...
    node_to_ptn_[id].type_ = new_type;
}
void t_rr_graph_storage::set_node_type_ptn(int id, t_rr_type new_type) {
    node_ptn_type_[id] = new_type;
}

void t_rr_graph_storage::set_node_dx_dy_ptn(int id, int dx, int dy) {
    node_ptn_[id].dx_ = dx;
    node_ptn_[id].dy_ = dy;
}
void t_rr_graph_storage::set_node_x_y(RRNodeId id, int x, int y) {
    node_to_ptn_[id].xlow_ = x;
    node_to_ptn_[id].ylow_ = y;
}
...
void t_rr_graph_storage::set_node_direction(RRNodeId id, Direction new_direction) {
    ...
    node_to_ptn_[id].dir_side_.direction = new_direction;
}
...
void t_rr_graph_storage::add_node_side(RRNodeId id, e_side new_side) {
    ...
    std::bitset<NUM_SIDES> side_bits = node_to_ptn_[id].dir_side_.sides;
    ...
    node_to_ptn_[id].dir_side_.sides = static_cast<unsigned char>(side_bits.to_ulong());
}
