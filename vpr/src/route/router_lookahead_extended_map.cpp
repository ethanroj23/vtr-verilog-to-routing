...
bool ExtendedMapLookahead::add_paths(RRNodeId start_node,
    ...
    for (auto it = path.rbegin(); it != path.rend(); it++) {
        ...
        int seg_index = device_ctx.rr_indexed_data[device_ctx.rr_graph.node_cost_index(RRNodeId(*it))].seg_index;
        ...
        if (size_t(this_node) != size_t(start_node)) {
            ...
            start_to_here = Entry(this_node, rr_graph.edge_switch(parent, paths[*it].edge), &start_to_here);
            parent = this_node;
        }
...
void ExtendedMapLookahead::compute(const std::vector<t_segment_inf>& segment_inf) {
    ...
    for (const auto& region : sample_regions) {
        ...
        std::vector<bool> node_expanded(device_ctx.rr_graph.size());
        std::vector<util::Search_Path> paths(device_ctx.rr_graph.size());
