...
std::pair<float, float> ExtendedMapLookahead::get_expected_delay_and_cong(RRNodeId from_node, RRNodeId to_node, const t_conn_cost_params& params, float /*R_upstream*/) const {
    ...
    if (!cost_entry.valid()) {
        ...
        VTR_LOGV_DEBUG(f_router_debug,
                       ...
                       size_t(from_node), device_ctx.rr_graph.node_type_string(RRNodeId(size_t(from_node))), from_seg_index,
                       size_t(to_node), device_ctx.rr_graph.node_type_string(RRNodeId(size_t(to_node))));
...
std::pair<float, int> ExtendedMapLookahead::run_dijkstra(RRNodeId start_node,
    ...
    while (!pq.empty()) {
        ...
        if (device_ctx.rr_graph.node_type(RRNodeId(size_t(node))) /*ESR API*/ == IPIN) {
