...
RRNodeId RoutingToClockConnection::create_virtual_clock_network_sink_node(int x, int y) {
    ...
    rr_nodes.set_node_ptc_num(node_index, ptc);
    rr_nodes.set_node_coordinates(node_index, x, y, x, y);
    rr_nodes.set_node_capacity(node_index, 1);
    rr_nodes.set_node_cost_index(node_index, SINK_COST_INDEX);
    rr_nodes.set_node_type(node_index, SINK);
    float R = 0.;
    float C = 0.;
    rr_nodes.set_node_rc_index(node_index, find_create_rr_rc_data(R, C));
