...
bool RouterDelayProfiler::calculate_delay(int source_node, int sink_node, const t_router_opts& router_opts, float* net_delay) {
    ...
    //VTR_LOG("Explored %zu of %zu (%.2f) RR nodes: path delay %g\n", router_stats.heap_pops, device_ctx.rr_graph.size(), float(router_stats.heap_pops) / device_ctx.rr_graph.size(), *net_delay);
...
std::vector<float> calculate_all_path_delays_from_rr_node(int src_rr_node, const t_router_opts& router_opts) {
    ...
    std::vector<float> path_delays_to(device_ctx.rr_graph.size(), std::numeric_limits<float>::quiet_NaN());
    ...
    VTR_ASSERT(shortest_paths.size() == device_ctx.rr_graph.size());
    for (int sink_rr_node = 0; sink_rr_node < (int)device_ctx.rr_graph.size(); ++sink_rr_node) {
