...
TEST_CASE("read_rr_graph_metadata", "[vpr]") {
...

    {
        ...
        for (int inode = 0; inode < (int)device_ctx.rr_nodes.size(); ++inode) {
            if ((device_ctx.rr_graph.node_type(RRNodeId(inode)) == CHANX || device_ctx.rr_graph.node_type(RRNodeId(inode)) == CHANY) && device_ctx.rr_graph.node_num_edges(RRNodeId(inode)) /*ESR API*/ > 0) {
                src_inode = inode;
                break;
            }
        }
        ...
        REQUIRE(src_inode >= 0);
        sink_inode = device_ctx.rr_graph.node_edge_sink_node(RREdgeId(0)) /*ESR API*/;
        switch_id = device_ctx.rr_graph.node_edge_switch(RREdgeId(0)) /*ESR API*/;
